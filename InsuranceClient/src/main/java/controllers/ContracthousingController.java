...
private ContractHousingContainerController containerParent;
    ...
    public void setContainer(ContractHousingContainerController container){
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		/*
		 * Node[] nodes = new Node[10]; for (int i = 0; i < nodes.length; i++) {
		 * try {
		 * 
		 * final int j = i; nodes[i] =
		 * FXMLLoader.load(getClass().getResource("ClaimboxView.fxml"));
		 * 
		...
		try {
			...
			Area.setCellValueFactory(cellData -> new SimpleStringProperty(
					cellData.getValue().getClient().getFirstName() + cellData.getValue().getClient().getFirstName()));
			type.setCellValueFactory(new PropertyValueFactory<>("guarantee"));
			prime1.setCellValueFactory(
					cellData -> new SimpleStringProperty(String.valueOf(cellData.getValue().getPrime())));
			...
			/*
			 * Image a1 = new Image("http://localhost/image/" +
			 * oblist.get(i).getPrime()); imageView.setImage(a1);
			 * imageView.setFitHeight(120); imageView.setFitWidth(150);
			 * 
			 * 
		...
		try {
			...
			for (int i = 0; i < oblist.size(); i++) {
				...
				if (oblist.get(i).getEtatdemande().equals("Refused")
						|| oblist.get(i).getEtatdemande().equals("Accepted")) {
					to++;
				}
			}
	...
	void onMouseClick(MouseEvent event) throws Exception {
		...
		cd.startstage();*/
		  ...
		  FXMLLoader loader = containerParent.switchViewTo("/views/ContractHousingDetailView.fxml");
	      ((ContractdetailController) loader.getController()).setContainer(containerParent);
	      ((ContractdetailController) loader.getController()).setid(id1);
