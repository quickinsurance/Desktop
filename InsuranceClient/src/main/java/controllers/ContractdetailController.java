...
public class ContractdetailController implements Initializable {
	...
	public void setid(int id) {
		...
		try {
			 ...
			 * 
			 * Image a1 = new Image("http://localhost/images/"
			 * +house.getImage()+".jpg"); System.out.println(house.getImage());
			 * image1.setImage(a1); image1.setFitHeight(200);
	...
	/*
	 * public void mapInitialized() { double lat = 0 ; //33.8869; double lon =
	 * 0; // 9.5375 ; MapOptions mapOptions = new MapOptions();
	 * mapOptions.center(new LatLong(lat, lon)) .mapType(MapTypeIdEnum.ROADMAP)
	 * .zoom(10);
	 * 
	 * map = mapid.createMap(mapOptions, false); MarkerOptions markerOptions =
	 * new MarkerOptions(); markerOptions.position(new LatLong(lat, lon))
	 * .visible(Boolean.TRUE) .title("My Marker"); if (marker == null) { marker
	 * = new Marker(markerOptions); map.addMarker(marker); } else {
	 * map.removeMarker(marker); marker = new Marker(markerOptions);
	 * map.addMarker(marker); }}
	 * 
	 * geocodingService = new GeocodingService(); MapOptions mapOptions = new
	 * MapOptions();
	 * 
	 ...
	 * .streetViewControl(false) .zoomControl(false) .zoom(12); String adresse =
	 * "tunisie"; map = googlemap.createMap(mapOptions);
	 * geocodingService.geocode(adresse, (GeocodingResult[] results,
	 * GeocoderStatus status) -> {
	 * 
	 * LatLong latLong = null; Marker mark; MarkerOptions markerOptions = new
	 * MarkerOptions(); System.out.println(results[0].getFormattedAddress());
	 * markerOptions.position( new
	 * LatLong(results[0].getGeometry().getLocation().getLatitude(),
	 * results[0].getGeometry().getLocation().getLongitude()))
	 * .visible(Boolean.TRUE) .title("My Marker");
	 * 
	 * Marker marker = new Marker( markerOptions );
	 * 
	 * map.addMarker(marker); if( status == GeocoderStatus.ZERO_RESULTS) { Alert
	 * alert = new Alert(Alert.AlertType.ERROR, "No matching address found");
	 * alert.show(); return; } else if( results.length > 1 ) { // Alert alert =
	 * new Alert(Alert.AlertType.WARNING,
	 * "Multiple results found, showing the first one."); //alert.show();
	 * latLong = new
	 * LatLong(results[0].getGeometry().getLocation().getLatitude(),
	 * results[0].getGeometry().getLocation().getLongitude()); } else { latLong
	 * = new LatLong(results[0].getGeometry().getLocation().getLatitude(),
	 * results[0].getGeometry().getLocation().getLongitude()); }
	 * 
	...
	void printPdfContract(ActionEvent event) throws DocumentException, IOException {
		...
		FileOutputStream file = new FileOutputStream(
				new File("C:\\Users\\haythem\\Desktop\\Contract\\" + l + f + "Contract.pdf"));
