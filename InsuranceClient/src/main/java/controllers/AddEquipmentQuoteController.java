...
public class AddEquipmentQuoteController implements Initializable {
	...
	public void setContainer(EquipmentQuotContainerController container) {
	...
	void AddQuote(MouseEvent event) throws NamingException {
		if (item.getSelectionModel().isEmpty() || make.getSelectionModel().isEmpty()
				|| modelField.getText().trim().isEmpty() || valueField.getText().trim().isEmpty()
				|| condition.getText().trim().isEmpty() || dateValue.getValue() == null
			...
			Notifications notif = Notifications.create().text("    Champs manquants").hideAfter(Duration.seconds(10))
					.position(Pos.CENTER);
			...
			if (x == null) {
				...
				for (int i = 0; i < xx.size(); i++) {
					if (xx.get(i).getCin() == Integer.valueOf(cinField.getText())
							|| xx.get(i).getPhone() == Integer.valueOf(phone.getText())) {
						existe++;
					}
				}
				if (existe == 0) {
	...
	void MergeQuote(ActionEvent event) throws NamingException {
		if (item.getSelectionModel().isEmpty() || make.getSelectionModel().isEmpty()
			...
			if (x != null) {
				...
				List<Quotation> quo = proxy1.findQuoteByTypeAndDate(
						type_item.getSelectionModel().getSelectedItem().toString(), x.getId(), datet.toString());
				...
				quo1.addAll(
						proxy1.findMerge(prop.get(0).getEquipmentQuote().getQuotation_id(), String.valueOf(date), x));
	...
	void addConract(ActionEvent event) throws NamingException {
		...
		if (addContractbtn.getText().equals("Add Contract") || addContractbtn.getText().equals("Add your pack")) {
			...
			if (addContractbtn.getText().equals("Add Contract")) {
				addContractbtn.setText("Validate Contact");
			} else {
			...
			if (item.getSelectionModel().isEmpty() || make.getSelectionModel().isEmpty()
				...
				if (client == null) {
					...
					List<Quotation> qq = proxy2.findQuoteByTypeAndDate(
							type_item.getSelectionModel().getSelectedItem().toString(), client.getId(),
							date.toString());
					...
					for (int i = 0; i < prop.size(); i++) {
						...
						c1.setOption_contract(
								c1.getOption_contract().valueOf(prop.get(i).getOption_quote().toString()));
					...
					List<Contract> quo = proxy1.findContractByType(
							type_item.getSelectionModel().getSelectedItem().toString(), client.getId());
					...
					quo1.addAll(proxy1.findMerge(proper.get(0).getEquipment().getContract_id(), String.valueOf(date),
							client));
					for (int j = 0; j < quo1.size(); j++) {
	...
	public void CalculatePrimeOneProperty(Property p, EquipmentQuote q) throws NamingException {
		...
		float xx = x;
		float nrisque = 500;
		if (p.getItem().equals("Camera") || p.getItem().equals("Games") || p.getItem().equals("MobilePhone")
				|| p.getItem().equals("EReader")) {
			nrisque = 1000;
		}
		...
		Notifications notif = Notifications.create().text("Your premium is :" + pr).hideAfter(Duration.seconds(10))
				.position(Pos.CENTER);
		notif.darkStyle().graphic(null);
		notif.show();
