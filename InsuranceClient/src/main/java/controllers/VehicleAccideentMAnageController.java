...
public class VehicleAccideentMAnageController implements Initializable {
	...
	public void Afficher(){
		...
		try {
			...
			context = new InitialContext();
			IAccidentInterfaceRemote proxy = (IAccidentInterfaceRemote)context.lookup(jndiName);
			List<Accident> accidents = proxy.getListVehicleAccident(agent);
			...
			insureName.setCellValueFactory(cellData->new SimpleStringProperty(cellData.getValue().getVehicle().getClient()
			...
			List<Expert> experts = proxyExpert.getExpertBySpeciality("Vehicle");
			...
			//table 2
		 	...
		 	List<Accident> accidentsTreated =proxy.getListVehicleAccidentTreatedByExpert(agent);
			...
			name2.setCellValueFactory(cellData->new SimpleStringProperty(cellData.getValue().getVehicle().getClient()
					.getFirstName()+ cellData.getValue().getVehicle().getClient().getLastName()));
		...
		if (result.get() == buttonTypeOne) {
			if (tblManagesinister.getSelectionModel().getSelectedItem() != null) {
				...
				/*try {
					...
					Stage primaryStage = new Stage();
					FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/ReportViewVehicle.fxml"));
					Parent root1 = (Parent) loader.load();
					primaryStage.setTitle("Report");
					Scene scene = new Scene(root1);
					scene.getStylesheets().add("/css/application.css");
					primaryStage.setScene(scene);
					ReportViewVehicleController mainController = loader.<ReportViewVehicleController>getController();
				  ...
				  FXMLLoader loader = containerParent.switchViewTo("/views/ReportViewVehicle.fxml");
			        ((ReportViewVehicleController) loader.getController()).setContainer(containerParent);
			        ((ReportViewVehicleController) loader.getController()).setId(selectedAccident.getAccident_id());
